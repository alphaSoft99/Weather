...
class UseInfoViewModel : ViewModel() {

    private lateinit var preference: PreferencesUtil
    private lateinit var api: PostApi

    private val _postLiveData = MutableLiveData<List<NewsData>>()
    val postLiveData: LiveData<List<NewsData>> = _postLiveData
    ...
    fun get(context: Context) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = api.getNews(preference.getUiLocale())
