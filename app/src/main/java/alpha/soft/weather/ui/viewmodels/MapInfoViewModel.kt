...
class MapInfoViewModel : ViewModel() {

    private lateinit var preference: PreferencesUtil
    private lateinit var api: PostApi

    private val _postLiveData = MutableLiveData<ManualData>()
    val postLiveData: LiveData<ManualData> = _postLiveData
    ...
    fun get(context: Context) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = api.getMapInfo(preference.getUiLocale())
                if (response.isSuccessful) {
                    _postLiveData.postValue(response.body()?.data)
                } else {
